...
cubemap::cubemap(const std::vector<std::string>& paths){
    ...
    for (unsigned int i = 0; i < paths.size(); i++) {
        ...
        unsigned char* image = SOIL_load_image(paths[i].c_str(), &width, &height, &nr_channels, SOIL_LOAD_AUTO);
        if (image == nullptr) {
...
void cubemap::set_active_texture(const shader* target_shader, int texture_unit, std::string name) const{
